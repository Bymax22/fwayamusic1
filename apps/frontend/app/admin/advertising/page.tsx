...
const apiBase = () =>
  `${process.env.NEXT_PUBLIC_API_URL || process.env.BACKEND_URL || "http://localhost:3001"}/api/v1/advertising`;
...
export default function AdvertisingAdminPage() {
  ...
  const load = async () => {
    ...
    if (response.ok) {
      ...
      const results = await Promise.all(
        loaded.map((campaign: Campaign) =>
          fetch(`${apiBase()}/campaigns/${campaign.id}/analytics`, {
            headers: headers(),
      ...
      setAnalytics(
        Object.fromEntries(
          results
            .map((item, index) => (item ? [loaded[index].id, item] : []))
            .filter(Boolean),
        ),
      );
    }
  ...
  useEffect(() => {
    ...
    void subscribe("advertising:updated", () => void load()).then(
      (unsubscribe) => {
        cleanup = unsubscribe;
      },
    );
  ...
  const addCreative = async (event: FormEvent) => {
    event.preventDefault();
    if (!creative.campaignId || !creative.file) return;
    setBusy(true);
    setMessage("");
    try {
      const body = new FormData();
      body.append("title", creative.title || "Sponsored advertisement");
      body.append("clickUrl", creative.clickUrl);
      body.append("mediaType", creative.mediaType);
      body.append(
        "placement",
        creative.placement,
      );
      body.append("file", creative.file);
      const response = await fetch(
        `${apiBase()}/campaigns/${creative.campaignId}/ads`,
      ...
      setMessage(
        error instanceof Error ? error.message : "Creative upload failed",
      );
    } finally {
      setBusy(false);
    }
  ...
  const toggleCampaign = async (campaign: Campaign) => {
    await fetch(`${apiBase()}/campaigns/${campaign.id}`, {
      method: "PATCH",
      headers: { ...headers(), "Content-Type": "application/json" },
  ...
  const removeCampaign = async (id: number) => {
    if (confirm("Delete this campaign and its creatives?")) {
  ...
  const removeCreative = async (id: number) => {
    await fetch(`${apiBase()}/ads/${id}`, {
      method: "DELETE",
      headers: headers(),
    });
    await load();
          ...
          <form onSubmit={addCreative} className="grid gap-3 md:grid-cols-6">
            ...
            <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-white/10 px-3 py-2 hover:bg-white/10">
              ...
              <input
                required
                type="file"
                accept={creative.mediaType === "VIDEO" ? "video/*" : "image/*"}
                onChange={(e) =>
                  setCreative({
                    ...creative,
