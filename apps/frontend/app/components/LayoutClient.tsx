...
const SidebarWithAuth = ({ sidebarExpanded }: { sidebarExpanded: boolean }) => {
  ...
  return (
    <aside
      id="sidebar"
      className={`sidebar fixed top-24 sm:top-16 left-0 h-[calc(100vh-6rem)] sm:h-[calc(100vh-4rem)] z-40 transition-all duration-300 ease-in-out ${sidebarExpanded ? "w-56" : "w-14"}`}
            ...
            <PaymentProvider>
              <AuthWrapper>
                <div className="flex flex-col h-screen">
                  ...
                  {/* Compact Navbar */}
                  <header className="fixed top-0 left-0 right-0 h-24 sm:h-16 z-50">
                    <Navbar />
                  </header>
                  
                  {/* Main content area */}
                  <div className="flex flex-1 pt-24 sm:pt-16"> {/* Navbar is fixed, add padding for navbar height */}
                    {user && (
