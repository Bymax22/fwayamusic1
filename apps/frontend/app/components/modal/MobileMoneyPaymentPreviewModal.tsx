          ...
          >
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">This is a 30s preview — purchase to continue listening.</p>
              </div>
              <button onClick={handleClose} className="text-gray-400 hover:text-white transition-colors p-1 rounded-lg hover:bg-[#0a3747]" disabled={isProcessing && step === 'processing'}>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="mb-6 p-4 bg-[#0a3747]/50 rounded-xl border border-[#0a4a5f]">
              ...
              <div className="mt-2 flex justify-between items-center">
                <span className="text-2xl font-bold text-[#e51f48]">{media.currency} {media.price.toFixed(2)}</span>
            ...
            {step === 'form' && (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">MTN Mobile Money Number</label>
                  <div className="relative">
                    <input type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="0961234567" className="w-full px-4 py-3 bg-[#0a3747] border border-[#0a4a5f] rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#e51f48] focus:border-transparent transition-all" disabled={isProcessing} />
                  ...
                  {errorMessage && (
                    <p className="text-red-400 text-sm mt-2 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errorMessage}</p>
                  )}
                  <p className="text-xs text-gray-400 mt-2">Enter your MTN Zambia number. You&apos;ll receive a USSD prompt to complete payment.</p>
                </div>

                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={handleClose} className="flex-1 px-4 py-3 border border-gray-600 text-gray-300 rounded-xl hover:bg-[#0a3747] transition-colors" disabled={isProcessing}>Cancel</button>
                  <button type="submit" disabled={isProcessing || !phoneNumber.trim()} className="flex-1 px-4 py-3 bg-gradient-to-r from-[#e51f48] to-[#ff4d6d] text-white rounded-xl hover:from-[#ff4d6d] hover:to-[#e51f48] disabled:opacity-50 disabled:cursor-not-allowed transition-all font-semibold">{isProcessing ? 'Processing...' : 'Pay Now'}</button>
                </div>
              </form>
            )}
            ...
            {step === 'processing' && (
              <div className="text-center py-8 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-gray-400 mb-4">Please check your phone for a USSD prompt...</p>
                </div>
                <div className="pt-4"><p className="text-xs text-gray-500">Transaction ID: {transactionId}</p></div>
              </div>
            )}
            ...
            {step === 'success' && (
              <div className="text-center py-6 space-y-4">
                ...
                <button onClick={handleClose} className="w-full px-4 py-3 bg-[#e51f48] text-white rounded-xl hover:bg-[#ff4d6d] transition-colors font-semibold">Start Listening</button>
              </div>
            )}

            {step === 'error' && (
              <div className="text-center py-6 space-y-4">
                <div className="w-20 h-20 bg-red-500 rounded-full flex items-center justify-center mx-auto"><AlertCircle className="w-10 h-10 text-white" /></div>
                <div>
                  <h3 className="text-white font-semibold text-lg mb-2">Payment Failed</h3>
                  <p className="text-gray-400 mb-4">{errorMessage}</p>
                </div>
                <div className="flex gap-3">
                  <button onClick={handleClose} className="flex-1 px-4 py-3 border border-gray-600 text-gray-300 rounded-xl hover:bg-[#0a3747] transition-colors">Cancel</button>
                  <button onClick={() => setStep('form')} className="flex-1 px-4 py-3 bg-[#e51f48] text-white rounded-xl hover:bg-[#ff4d6d] transition-colors font-semibold">Try Again</button>
                </div>
              </div>
            )}
          </motion.div>
