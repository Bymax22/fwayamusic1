...
type Provider = 'MTN_MONEY' | 'AIRTEL_MONEY' | 'ZAMTEL_MONEY';

const PLANS: Plan[] = [
  { id: 'DAILY', name: 'Daily', price: 50, duration: '24 hours', description: 'A quick premium pass' },
  { id: 'WEEKLY', name: 'Weekly', price: 180, duration: '7 days', description: 'Premium for the week' },
  { id: 'MONTHLY', name: 'Monthly', price: 450, duration: '30 days', description: 'Best for regular listeners' },
  { id: 'YEARLY', name: 'Yearly', price: 1450, duration: '365 days', description: 'The best annual value' },
];
...
export default function SubscriptionModal({ isOpen, onClose, onSuccess }: SubscriptionModalProps) {
  ...
  const [provider, setProvider] = useState<Provider>('MTN_MONEY');
  const [phoneNumber, setPhoneNumber] = useState('');
  const [step, setStep] = useState<'form' | 'processing' | 'success' | 'error'>('form');
  const [error, setError] = useState('');

  useEffect(() => {
    if (isOpen) { setStep('form'); setPhoneNumber(''); setError(''); setProvider('MTN_MONEY'); }
  }, [isOpen]);

  const submit = async (event: React.FormEvent) => {
    event.preventDefault();
    const normalized = phoneNumber.replace(/\s/g, '');
    if (!/^(260|0)?[2579]\d{8}$/.test(normalized)) {
      setError('Enter a valid Zambian mobile-money number, for example 0961234567.');
      return;
    }
    ...
    try {
      ...
      const transaction = await initiateSubscriptionPayment(user.id, selectedPlan.id, phone, selectedPlan.price, 'ZMW', provider);
      await processMobileMoneyPayment(transaction.transactionId, phone, provider);
      await refreshUser();
      setStep('success');
      onSuccess?.();
    } catch (caught) {
  ...
  return <AnimatePresence>{isOpen && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
    <motion.div initial={{ opacity: 0, y: 20, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20 }} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-black p-5 text-white shadow-2xl sm:p-7">
      <div className="flex items-start justify-between"><div><div className="mb-2 flex items-center gap-2 text-purple-300"><Crown size={18} /><span className="text-sm font-semibold uppercase tracking-widest">Fwaya Premium</span></div><h2 className="text-2xl font-bold">Choose your listening plan</h2><p className="mt-1 text-sm text-white/60">Choose a mobile-money provider to complete payment.</p></div><button onClick={onClose} aria-label="Close subscription modal" className="rounded-full p-2 text-white/60 hover:bg-white/10 hover:text-white"><X size={20} /></button></div>
      {step === 'success' ? <div className="py-12 text-center"><Check className="mx-auto mb-4 rounded-full bg-emerald-500/20 p-3 text-emerald-400" size={64} /><h3 className="text-xl font-bold">Subscription active</h3><p className="mt-2 text-white/60">Your {selectedPlan.name.toLowerCase()} plan is ready. Enjoy Fwaya Premium.</p><button onClick={onClose} className="mt-6 rounded-full bg-purple-600 px-6 py-3 font-semibold hover:bg-purple-500">Start listening</button></div> : <form onSubmit={submit} className="mt-6 space-y-5">
        ...
        <div><span className="mb-2 block text-sm font-medium">Payment provider</span><div className="grid grid-cols-3 gap-2">{([['MTN_MONEY', 'MTN'], ['AIRTEL_MONEY', 'Airtel'], ['ZAMTEL_MONEY', 'Zamtel']] as const).map(([value, label]) => <button type="button" key={value} onClick={() => setProvider(value)} className={`rounded-xl px-3 py-2.5 text-sm font-semibold transition ${provider === value ? 'bg-purple-600 text-white' : 'bg-white/10 text-white/60 hover:bg-white/15 hover:text-white'}`}>{label}</button>)}</div></div>
        <label className="block"><span className="mb-2 block text-sm font-medium">{provider === 'MTN_MONEY' ? 'MTN' : provider === 'AIRTEL_MONEY' ? 'Airtel' : 'Zamtel'} Mobile Money number</span><div className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-4"><Phone size={18} className="text-purple-300" /><input value={phoneNumber} onChange={event => setPhoneNumber(event.target.value)} placeholder="096 123 4567" className="w-full bg-transparent py-3 outline-none" inputMode="tel" disabled={step === 'processing'} /></div></label>
